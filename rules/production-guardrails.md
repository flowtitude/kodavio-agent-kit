...
4. Verificación post-write obligatoria: read-back + page health / editor-open check + **comprobación del render** cuando el cambio es visual (`rules/render-verification.md`: escrito ≠ visible). **Contrato de cierre**: una tarea NO puede declararse PASS sobre un write en producción sin output explícito de `wp-verifier` registrado en `sites/{slug}/NOTAS.md` (sección Verificación con timestamp + IDs read-back + veredicto PASS/PASS-avisos/FAIL). Si trabaja un solo agente, el verifier corre como **pase separado** (otra invocación, no en la misma respuesta del write). Sin ese registro la tarea queda `in_progress`, no se cierra.
...
## Cómo se deshace cada escritura

«Rollback con el backup ID» depende de qué se escribió. Todo con ensayo primero (`dry_run`, y leer
qué dice que restauraría).
...
| Plugin activado | `kodavio/wp-deactivate-plugin` (también es GATE en producción). No hay copia del estado anterior; quitar uno instalado es borrar: GATE |

Si el resultado de `kodavio/rollback` trae `error` en alguna operación, **esa no se ha deshecho**
aunque el resumen cuente otras: se dice cuál y se para.
