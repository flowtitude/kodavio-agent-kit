...
3. **Lo que ya existe**: `kodavio/content-list-post-types` y `kodavio/content-list-taxonomies` (el modelo actual), `kodavio/wp-list-posts` por tipo (cuántas entradas tiene y si son reales o de muestra). No se diseña encima de un modelo que no has leído.
...
- **Loops**: la consulta se prueba con `kodavio/query-preview` antes de entregarla — si no devuelve las entradas que esperas, el loop saldrá vacío. La que se va a reutilizar se guarda con `kodavio/query-save-preset` (ensayo, luego real) y se entrega por su nombre; `kodavio/query-list-presets` para no duplicar una que ya exista.
